...
export const RATE_REPORT = { windowMs: 60 * 60 * 1000, max: 20 };          // 举报：每 uid 20 条/小时（M4）
export const REPORT_REASON_MAX = 200;                                      // 举报说明长度（=表 reason ≤200）

/* 发消息（DO IMRoom 内生效，方案 §8） */
export const SEND_WINDOW_MS = 10_000;      // 每会话每 uid 滑窗
export const SEND_MAX_PER_WINDOW = 10;     // 10 条/10s
export const NEW_ACCT_SEND = { windowMs: 60_000, max: 5, ageMs: 60 * 60 * 1000 }; // 注册<1h 新号 5 条/min（M4）

/* 用户级在线（P1，docs/IM在线与实时同步方案.md） */
export const PRESENCE_WINDOW_MS = 90_000;         // 在线窗口：last_ping 距今内视为在线（读时计算，断线自愈）
export const PRESENCE_HEARTBEAT_MS = 25_000;      // 客户端心跳间隔（窗口内 ≥3 次成功心跳余量）
export const PRESENCE_WRITE_THROTTLE_MS = 20_000; // DO 心跳落库节流（每用户 D1 写 ≤3 次/min）
export const PRESENCE_UIDS_MAX = 100;             // 单次查询 uid 上限（=群上限；服务端按 40 分块查询）
export const PRESENCE_POLL_MS = 30_000;           // 聊天窗打开期间在线状态轮询间隔
