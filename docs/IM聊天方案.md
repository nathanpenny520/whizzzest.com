...
> 文档版本：**v1.5** ｜ 2026-09-14 ｜ 状态：**M1-M4 全部上线**（板块完结；M4=举报闭环/新号限流/安全数字/限流分键加固，见上线记录）
> v1.5 变更（在线与同步域独立立项）：用户级在线 + 实时同步改造拆分为独立方案文档 [IM在线与实时同步方案.md](./IM在线与实时同步方案.md)（业主拍板：登录即在线/P0 先行/P2 轻量 activity 通知/P3 暂不做）——§5 帧协议自此新增**存在性通道**（`/ws` 不带 conv 参数 → 用户 DO IMPresence：hello/pong，25s 心跳）与 `GET /api/presence?uids=`（仅好友/同会话可见，在线=last_ping 90s 窗口读时计算），在线口径从「开着本会话的 WS」升级为「登录 IM 即在线 + 房间实时集并集」；迁移 007（im_presence）+ wrangler DO 迁移 v2（IMPresence）。
> v1.4 变更（M4 实施落档）：① **举报落地**（§6/§8）——POST /api/reports（成员校验、非成员同回 404 防会话存在性探测、理由 1-200、seq 区间校验、每 uid 20 条/时）+ 前端 dm/群菜单「举报」；举报人自愿附本地解密引用文 v1 未做（只收文字说明）；② **新号限流落地**（§8「注册<1h 5条/min」）——DO IMRoom 内按 uid 查 im_users.created_at 判定（实例级缓存），per-conv 粒度与全局每 IP 限流叠加；③ **安全数字定稿**（§3.4 可选项转正）——1v1 聊天窗头部展示双方公钥按字节序联合 SHA-256 的安全数字（30 位十六进制 6 组，ui.js `safetyNumber`），替代原单侧指纹展示；群成员面板逐员指纹保留；④ **全局限流器按发送者 IP 分键**（顺修）——原 IMRoom 整会话共用首个发送者的 IP 建键，同会话他人会互相消耗限流预算；⑤ 顺修 chat.js dm 菜单 `blocked` 未声明（严格模式 ReferenceError，拉黑/删除按钮自 M2 起渲染不出，M2 验收走 contacts 视图未暴露）；⑥ §13 模块表增 `src/api/reports.js`。
...
| M4 | 加固+上线：RateLimiter DO + 全限流 + 举报拉黑 + 重连多标签 + 安全数字（可选）+ **主站导航「焰境万象」加入口** + 文档/上线记录 ✅ 2026-09-14（导航入口由独立任务提前完成 ceb0333） | 2-3 天 | 限流命中；举报落库可查；nav 入口 zh 生效（EN 随英文版批次后补） |
...
| `src/api/reports.js` | 举报（M4）：成员校验、理由/seq 区间约束、限流，落 im_reports |
