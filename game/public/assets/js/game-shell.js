...
async function boot() {
  ...
  try {
    ...
    // 键盘操作提示——按款标记 meta.kbHint（2026-09-10 业主拍板收窄：仅魔塔50层3D 一款启用，
    // 不再挂 keyboardOnly 全量生效；其余纯键盘游戏业主认为有鼠标/触屏可用，不弹）：
    // 进页即提示一次（快加载可见）；首次点击游戏区再补一次——慢加载游戏进页那条早停了，恰好点菜单没反应时才最需要。
