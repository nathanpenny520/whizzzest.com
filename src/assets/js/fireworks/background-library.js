/**
 * background-library.js — 背景库：IndexedDB（whizzzest_fireworks_bg）存本地上传背景图，
 * 记录含缩略图 data URL 与原图 blob，上限 12 张。
 */
...
(function initFireworksBackgroundLibrary(global) {
  ...
  function openDb() {
    ...
    dbPromise = new Promise((resolve, reject) => {
      ...
      request.onerror = () =>
        reject(
          request.error ||
            new Error(fwT('galleryOpenFailed', '无法打开本地图片库'))
        );
      request.onblocked = () =>
        reject(new Error(fwT('galleryBlocked', '本地图片库被其他页面占用')));
    }).catch((error) => {
      dbPromise = null;
      throw error;
    });
  ...
  function runInTransaction(db, mode, run) {
    return new Promise((resolve, reject) => {
      ...
      transaction.onabort = () =>
        reject(
          transaction.error ||
            new Error(fwT('galleryFailed', '本地图片库操作失败'))
        );
      transaction.onerror = () =>
        reject(
          transaction.error ||
  ...
  async function listImages() {
    ...
    const records = await runInTransaction(db, 'readonly', (store) =>
      store.getAll()
    );
  ...
  function decodeViaImageElement(file) {
    return new Promise((resolve, reject) => {
      ...
      image.onerror = () => {
        URL.revokeObjectURL(objectUrl);
        reject(
          new Error(fwT('imageReadFailed', '无法读取这张图片，请换一张试试'))
        );
      };
      image.src = objectUrl;
    });
    ...
    return new Promise((resolve, reject) => {
      canvas.toBlob(
        (blob) => {
          ...
          reject(
            new Error(fwT('imageProcessFailed', '图片处理失败，请换一张试试'))
          );
        },
        type,
        quality
  ...
  function createThumbDataUrl(sourceCanvas) {
    const scale = Math.min(
      1,
      THUMB_EDGE / Math.max(sourceCanvas.width, sourceCanvas.height)
    );
  ...
  async function addImage(file) {
    ...
    if (file.size > LIMITS.maxFileSize) {
      throw new Error(
        fwT('imageTooLarge', '图片文件过大，请选择 30MB 以内的图片')
      );
    }

    const db = await openDb();
    const existing = await runInTransaction(db, 'readonly', (store) =>
      store.getAll()
    );
    if (existing.length >= LIMITS.maxImages) {
      throw new Error(
        fwT('galleryFull', '最多保存 {n} 张背景图，请先删除部分图片').replace(
          '{n}',
          String(LIMITS.maxImages)
        )
      );
    }
