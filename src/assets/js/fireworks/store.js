/**
 * store.js — 设置状态机：localStorage 读写、版本迁移与枚举校验（画质/缩放/照亮/背景/文字烟花）。
 */
...
(function initFireworksAppStore(global) {
  ...
  const qualityValues = new Set(
    Object.values(appConfig.qualityLevels).map(String)
  );
  const skyLightingValues = new Set(
    Object.values(appConfig.skyLightingModes).map(String)
  );
  const scaleFactorValues = new Set(
    appConfig.scaleFactorOptions.map((value) => value.toFixed(2))
  );
  ...
  function normalizeBackground(
    ...
    const value =
      typeof rawBackground.value === 'string' ? rawBackground.value.trim() : '';
    if (!value) {
      return {
        mode: 'none',
        value: '',
    ...
    return {
      mode:
        rawBackground.mode === 'style'
          ? 'style'
          : rawBackground.mode === 'library'
            ? 'library'
            : 'image',
      value,
      configured:
    ...
    return {
      quality: String(
        runtime.isHighEndDevice
          ? appConfig.qualityLevels.high
          : appConfig.qualityLevels.normal
      ),
      shell: 'Random',
  ...
  function readLegacyState(defaultState) {
    ...
    try {
      ...
      const parsedSize =
        typeof rawSize === 'string' ? JSON.parse(rawSize) : null;
  ...
  function readStoredState(defaultState) {
    ...
    try {
      ...
      if (
        parsedState.schemaVersion === '2.1' ||
        parsedState.schemaVersion === '2.0'
      ) {
        return {
          ...defaultState,
          config: normalizeLegacyWordShellConfig(
            parsedState.data.config,
            defaultState.config
          ),
          background: normalizeBackground(
            parsedState.data.background,
      ...
      if (
        parsedState.schemaVersion === '1.2' ||
        parsedState.schemaVersion === '1.1'
      ) {
        return {
          ...defaultState,
          config: normalizeLegacyWordShellConfig(
  ...
  function createStore(defaultState, shouldLoad) {
    const initialState = shouldLoad
      ? readStoredState(defaultState)
      : defaultState;
    return {
      _listeners: new Set(),
      state: initialState,
