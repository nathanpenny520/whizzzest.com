/**
 * stage.js — 画布舞台：Ticker 帧循环、高 DPI 缩放、指针/触摸事件分发（事件挂 document，
 * closest('.fw-app') 守卫防站点导航点击被映射成画布坐标误触热区）。
 */
...
const Stage = (function StageFactory(window, document, Ticker) {
  ...
  Stage.mouseHandler = function mouseHandler(event) {
    // 站点导航在画布容器之外；指针事件挂在 document 上，不拦会把点导航映射成画布负坐标、误触顶部热区
    if (!event.target || !event.target.closest('.fw-app')) {
      return;
    }
    ...
    Stage.stages.forEach((stage) => {
      const position = Stage.windowToCanvas(
        stage.canvas,
        event.clientX,
        event.clientY
      );
  ...
  Stage.touchHandler = function touchHandler(event) {
    // 同 mouseHandler：只处理发生在模拟器容器内的触摸
    if (!event.target || !event.target.closest('.fw-app')) {
      return;
    }
      ...
      Array.from(event.changedTouches).forEach((touch) => {
        ...
        if (type !== 'end') {
          ...
          if (type === 'start') {
            stage.pointerEvent(
              'move',
              position.x / stage.dpr,
              position.y / stage.dpr
            );
          }
        } else {
