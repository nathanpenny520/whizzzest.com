/**
 * ui.js — 设置面板：DOM 查询、控件渲染与事件绑定（类型/大小/画质/背景库画廊等）。
 */
...
(function initFireworksAppUI(global) {
  ...
  function populateControls(nodes, shellNames, options) {
    ...
    nodes.skyLighting.innerHTML = createOptionMarkup(
      options.skyLightingOptions
    );
    nodes.scaleFactor.innerHTML = createOptionMarkup(
  ...
  function renderApp(state, nodes) {
    ...
    nodes.backgroundInput.value =
      state.background.configured && state.background.mode !== 'library'
  ...
  function renderBackgroundGallery(nodes, items, selectedId) {
    ...
    items.forEach((item) => {
      ...
      thumbButton.setAttribute(
        'aria-label',
        fwT('useBg', '使用背景图 {name}').replace('{name}', item.name)
      );
      ...
      deleteButton.setAttribute(
        'aria-label',
        fwT('deleteBg', '删除背景图 {name}').replace('{name}', item.name)
      );
  ...
  function bindAppControls(options) {
    ...
    nodes.backgroundClearBtn.addEventListener(
      'click',
      options.onBackgroundClear
    );

    nodes.backgroundUploadInput.addEventListener('change', () => {
      const file =
        nodes.backgroundUploadInput.files &&
        nodes.backgroundUploadInput.files[0];
